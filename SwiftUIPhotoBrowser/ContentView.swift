...
struct ContentView: View {
    ...
    var body: some View {
        ...
        VStack {
            Button("保存图片") {
                photoBrowser.saveAllImagesToDocumentDirectory()
            }
            Button("保存视频") {
                photoBrowser.saveAllVideosToDocumentDirectory()
            }
            Button("保存动图"){
                photoBrowser.saveAllGifToDocumentDirectory()
            }
            Button("选择多个照片") {
                photoBrowser.reset()
                ...
                if photoBrowser.videoImages != []{
                    HStack {
                        ForEach(0..<photoBrowser.videoImages.count, id: \.self) { i in
                            NavigationLink(
                                destination: VideoPreviewView(photoBrowser: photoBrowser, index: i))
                                {
                                    ZStack{
                        ...
                        photoBrowser.getVideoAsstetsUrlToString()
                    })
                }
            }
            ScrollView(.horizontal, showsIndicators: false) {
